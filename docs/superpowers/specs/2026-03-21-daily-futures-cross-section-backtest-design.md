...
## 回测插件可复用性
可复用部分（建议抽到通用核心层）：
- 成本模型：手续费、滑点、合约乘数、最小变动单位。
- 撮合与成交：成交价计算、成交量取整。
- 持仓与 PnL：现金、保证金占用、净值与收益计算。
- 指标统计：净值、回撤、年化、换手率等。

需要适配的部分（由策略/频率层负责）：
- 触发机制：日频依赖 `EVENT_EOD`，其他频率需自定义触发点。
- 输入语义：日频使用“调仓意图”，其他策略可能是逐笔订单或目标权重。
- Universe/roll：期货主力切换是特定逻辑，其他资产可替换为固定池或自定义过滤。
...
- `backtest_core`（通用引擎）：
  - 接收标准化的“成交请求/目标仓位”结构。
  - 提供撮合、成本、持仓、指标输出。
- `strategy_adapter`（策略/频率适配层）：
  - 将策略输出转为标准化请求。
  - 定义触发时机与时间对齐（如 `trade_lag`）。

标准化输入建议字段（MVP）：
- `symbol`, `side`, `volume_delta`
- `exec_day`, `price_basis`
- `meta`（可选：来源因子、权重、排序分数）
