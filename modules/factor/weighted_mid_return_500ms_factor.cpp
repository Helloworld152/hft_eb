...
double safe_weighted_mid(const TickRecord* tick) {
    if (!tick) return 0.0;
    double bid = tick->bid_price[0];
    double ask = tick->ask_price[0];
    double bid_vol = static_cast<double>(tick->bid_volume[0]);
    double ask_vol = static_cast<double>(tick->ask_volume[0]);
    double denom = bid_vol + ask_vol;
    if (bid > 0.0 && ask > 0.0 && denom > 0.0) {
        return (bid * ask_vol + ask * bid_vol) / denom;
    }
...
class WeightedMidReturn500msFactor : public IFactorNode {
public:
    ...
    double compute(const FactorContext& ctx, const std::vector<double>& /*inputs*/) override {
        ...
        double mid = safe_weighted_mid(ctx.tick);
...
EXPORT_FACTOR(WeightedMidReturn500msFactor)
